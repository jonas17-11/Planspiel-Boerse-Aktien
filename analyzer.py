...
# Einfache Prognose: letzten Close-Wert für nächsten Tag
def simple_forecast(df):
    forecast_df = pd.DataFrame(index=pd.date_range(start=df.index[-1]+pd.Timedelta(days=1), periods=1))
    forecast_df['Predicted'] = df['Close'].iloc[-1]
    return forecast_df
